...
foreach type in PR BR {
	foreach file in `type'1.dta `type'2.dta `type'3.dta `type'4.dta {
		rm $TEMP/`file'
	}
}

use $TEMP/twins1
append using $TEMP/twins2 $TEMP/twins3 $TEMP/twins4
...
foreach country of varlist country1-country60 {
	dis "`country'"
	foreach num of numlist 6(1)39 {
		egen school_zscore_`num'=std(educ) if age==`num' & `country'==1
	}
	egen school_zscore_`country'=rowtotal(school_zscore_6-school_zscore_39)
	drop school_zscore_6-school_zscore_39
}
egen school_zscore=rowtotal(school_zscore_country1-school_zscore_country60)
