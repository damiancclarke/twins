...
cap program drop plausexogTest
program plausexogTest, eclass
version 8.0
#delimit ;
...
********************************************************************************
*** (5b) Local to Zero approach (ltz) with non-standard distribution
********************************************************************************
if length("`nonstandard'")!=0 {
    ...
    foreach i of numlist 1(1)`nsims' {
        drawnorm cov(e(V)) means(e(b)), n(1)
        drop __drewnorm*
    }
}
...
if length("`graph'")!=0 {
	...
	if "`method'"=="ltz" {
		...
		scheme(s1color) `graphopts'                                          ///
		xline(0.0091, lcolor(red) lpattern(dash) lwidth(thick))
