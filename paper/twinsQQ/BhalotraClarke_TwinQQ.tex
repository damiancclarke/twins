...
$i$. %In the discussion that follows we assume without loss of generality that a higher value of $quality_{ij}$ implies a higher ``quality'' child based on the human capital measure of interest, so that
...
using OLS %with cross-sectional data 
%DD It's not just with CS data is it?
%%S: I guess you have a particular case in mind?  When I thought quickly of the big QQ papers like Roz Wolpin, Black et al, and Angrist et al, they either use survey data or census data at one point in time.  I don't think it matters that we remove the CS mention in any case! 
...
\citep{Qian2009}. %The is well-known omitted variable bias implies that OLS estimates of the quantity-quality trade-off will fail to recover the underlying parameter $\alpha$ unless all relevant variables are included in $\bm{X}$. 
...
%DD strictly i think negative selection will refer to mothers with a lower taste for (or capacity for?) investment in children. if this is correlated with the mother's level of HK then what you say will hold but that is another assumption that we dont need? please check the next sentence where i replace mother capital with mother preferences.do you want me to add after weaker preferences anything about weaker ability to invest? i think no but i havent read the textbook lit for a long time!
%%SB: This seems like a correct and nice way to state it to me.  When I wrote below I was thinking of the basic omitted variable bias which has that beta_1 hat = beta_1 + delta*beta_2, so the bias is delta (the effect of the omitted variable on the included variable, and beta_2, the effect of the omitted variable on the outcome of interest.  However, your sentence nicely bundles beta_2 and delta into one term!   
If mothers with weaker preferences for child quality have more children 
%and if the variables which define (negative) selection into fertility in turn
%have a negative effect on eventual child quality, 
...
%%DD i am not sure if i should drop all of the syntax around Neg Sel Fert so left it
%%SB: Thanks.  All good.
\begin{eqnarray}
...
\end{eqnarray}
%DD
%SB - Updated formatting.

%Given that a significant Q--Q trade-off implies a negative $\alpha$, negatively selected fertility will result in a \emph{more} negative estimate of $\alpha$ than the true value, and so overstate in magnitude the impact of additional children on a sibling's human capital outcomes.
...
\begin{eqnarray}
  %%DD i am dropping display of Pos Sel Fert since our est and prev est suggest neg sel and it is cleaner esp with pos selected twinning below to see the OLS-iV bounds idea. we still do acknowledge that this wil be reversed if fert is pos selected but no need to display that odd case.
  %SB - great.  This seems cleaner.
...
%The logic of these movements is that as additional relevant controls are included in the OLS regression, the remaining bias will become less severe, eroding the initial bias.  However, unless \emph{all} relevant fertility-selection variables are controlled for, bias will still remain in the parameter $\hat\alpha_1^{S+H}$.  This is summarised in the following two (testable) hypotheses:
...
%%Damian i think it is ok since "`population"' implies no selection but my instinct was to replace population value with true value- what do you think? 
...
%DD i added a sentence at the end of the prev para instead.
%The impossiblity of observing all relevant fertility-selection variables implies that OLS estimates controlling for observables will not result in unbiased parameter estimates for $\alpha_1$.  What's more, the size of the bias can not be determined without imposing some additional assumption about the degree of selection on unobservables.
%SB: Thanks.
%This has lead to the widespread use of instrumental variables in empirical estimates of the Q--Q model.  
%%DD i have added new ref BTV2016 which is the old sulfa-fertility paper that we are picking up with a new coauthor we invited just to get the final tables in shape so the citation is: Bhalotra, S., S. Telaligac and A Venkataramani. Fertility, Mortality Risk and Returns to Human Capital:Quasi-Experimental Evidence from 20th Century America. Mimeo. U of Essex.
%%SB: Done
...
births on the premise that they constitute an exogenous shock to family size.\footnote{Other approaches include instrumenting with the gender mix of the first two children \citep{AngristEvans1998,ConleyGlauber2006}, or exploiting policy experiments such as the introduction and subsequent relaxation of the one child policy in China
  \citep{Qian2009,ArgysAverett2015}, the eradication of hookworm \citep{BleakleyLange2009} and the introduction of antibiotics in the US \citep{Bhalotraetal2016}}.
	%DD i checked this paper and they do not have an IV for fertility so i think it is not appropriate as a ref here-- damian please can you delete from bibl.
	%fertility transitions during industrialisation \citep{DalgaardStrulik2015}.}  
%%DD where we cite conley-glauber should we not also cite Angrist-Evans i thought the same sex iv was theirs?
%SB: Thanks, added this cite first.
...
%The premise of part I of this paper is that twin births are not random. As documented, the higher biological demands placed on a mother pregnant with twins, results in a greater likelihood of that healthy or health-seeking mothers take twin conceptions to term.  
...
% The inconsistency of the IV estimate depends in part on the degree of the correlation between the likelihood of twinning and the stochastic error term $\eta_{ij}$ from equation \ref{TWINeqn:IV1}.  %Positively selected twinning---where mothers who
...
\end{equation}
%given that mothers are positively selected into higher fertility (via twinning), and similarly positively selected into investing more in their children.
...
%%DD wherever we say IV i have changed to twin-IV
%DD -- everywhere- please check if we want to say population value or true value.
%SB: I would have used these two terms interchangably.  Am I wrong in using population value to mean true value?  I always meant true value.  I think we are fine either way, but if you think we should prefer one or the other I can change.
...
%%DD pl check changed title below. we already discussed adding observables will move parameter towards true value so just taking stock below.
%SB: Good change.  Thanks!
\subsection{Bounds Derived by Bias Adjustment}
%Thus, if all relevant fertility- or twin-selection variables could be observed, an unbiased or consistent parameter could be estimated by OLS or IV.  However, given the difficulty in fully controlling for observables, both sets of estimates will unlikely converge on the population parameter of interest. 
%As we document in the following sections of this paper, mothers with twins are positively selected over a wide range of health behaviours and socioeconomic measures.
...
\]
%In this case, controlling for the (negative) selection variables in OLS will push the OLS estimate towards a tighter and tighter lower bound, and controlling for the (positive) selection variables in IV will result in a tighter upper bound. This gives the following hypothesis.
...
%%xxxxDCC:start-here
%%DD I think this may be the place to briefly introduce Altonji, Oster bounds-- we can refer to them- our main point will to state what the assumptions of this bounding are and to link it into the disucssion above. We have already explained that introducing controls like H and S will reduce the OLS trade off. If unobservables act "`proportionally to"' observables then if we could adjust for unobservables, we would further reduce the tradeoff. We want an estimte of how much. Similarly Oster. we should be poised at end of results section to be able to state the 3 sorts of bounds (a-c below) and how they overlap/compare and what our inference is from these diff approaches.
%SB: I agree. I have flagged this in red also in the text and I will draft and include.
...
%%DD please check change of title below - i put IV in the title on purpose to distinguish from OLS-bounds and from the OLS-IV bounding. i thought one way to organize thoughts on the different bounding exercises is a)) OLS-IV provide bounds;  b)) Bounds on OLS provided by Altonji, Oster ; c)) Bounds on IV provided by Conley. It would be good to formalize this at least partially. For eg one natural Q is how the OLS-bounds and the IV-bounds are expected to locate themselves in the range suggested by the OLS-IV bounds. Do they push these bounds inwards or outwards? or can we not say // i suppose we can say once we specify the direction in which controls influence the estimates.
%SB: Title sounds good including IV.  I wrote the following before seeing that you say something similar below [I have flagged it with ****].  I agree on points (a) and (c) above.  However, I don't think that we can do any bounding with Altonji/Oster.  All Altonji et al and Oster purport to do (based on my recent reading of Oster and older memories of Altonji) is ask how much selection on unoservables would have to be for the parameter estimate to be larger than zero.  This is sort of the inverse of bounding, as they impose a lower bound on the paramter estimate as zero, and ask, 'how extreme would the unobservable bit have to be for this to be true?'  On the other hand, the Conley et al work actually is bounding, as we impose a value for the unobservable bit, and based on this explicitly calculate a maximum and minimum for our parameter of interest.  I'm not sure if we can actually turn (b) on its head to get a similar bounds-style idea without doing some serious theoretical work.

\subsection{Estimating IV Bounds under Plausible Exogeneity}
\label{TWINscn:gamma}
%In the previous subsection, Q--Q estimation using twins is motivated in equations (\ref{TWINeqn:IV1}) and (\ref{TWINeqn:IV2}).  

%%DD i rewrote this section more than the above.
...
\end{equation}
%%SB: Was the below meant to be in here as a comment but not in the paper?  I think at least a bit of one sentence comes directly from the Conley et al paper (the dogmatic prior belief bit). I guess we took this for reference but need to re-write?
...
%However, even in cases such as this where we are not confident that $\gamma=0$, we can still estimate bounds on the Q--Q tradeoff if we are confident in making some statement of prior belief about the distribution from which $\gamma$ is drawn.  We invoke this terminology here, and refer to twins as a plausibly exogenous event, implying that we have reason to believe that $\gamma$ may be close to, but not necessarily precisely equal to, zero. Specifically, we are concerned that healthier mothers are more likely to give birth to twins, and, all else constant, healthier mothers are more likely to be able to invest more in their children post-pregnancy.
...
%%DD I just want to be sure that we can claim that our estimate of gamma is unbiased. i am not sure. shall we drop the word unbiased below?
%%SB: I would probably be slightly happier to say unbiased, though feel free to remove if it raises flags for you.  I prefer to state unbiased, as if we use a biased estimate of gamma then the bounds won't be necessarily correct.  Note that here all we are stating is that *if* we do have an unbiased estimate, this lets us proceed.  We then justify our gammas as unbiased estimates given that we get them from diff-in-diff in different places.
%%DD I also want to check on use of the word consistent below: is it accurate? Are estimates of bounds consistent? If yes shall we say consistently estimate bounds or shall we say estimate consistent bounds? I think both are ok ie use of unbaised gamma and consistent bounds but want to be sure as it is important.
%SB: Good point on consistent bounds!  I just went through Conley et al again and found no mention of consistent bounds, so I think this is probably incorrect terminology.  I have removed consistent, though left unbiased when referring to estimates of gamma.
...
%As the discussion above suggests, valid estimation of $\gamma$ requires an exogenous health shock to the mother, which can then be linked to the eventual quality of her children.  The data requirements in this circumstance are quite demanding, as we must observe mothers differentially affected by a health shock, and the eventual effect on their child quality.

% Fortunately in the US, a valid candidate exists using historic exposure to antibiotics, and following mothers over time using publicly released census data.  

%%DD i have added ref to Lesch cited in our sulfa1 paper.
%SB: Thanks -- incorporated.
%%DD I realized a potential problem- in our BV sulfa paper we find no impacts of sulfa at birth on women! only on men. however in unpublished results we found that sulfa women have higher fertility. this could be fecundity (since they are not more educated it seems not to be the result of an increased opportunity cost of their time!). if fecundity is a func of health i suppose we could push this..but we have no direct measure of health. (also note that we found higher fert of these women and you find higher educ so something is shifting the QQ indifference curve out. interesting. see what i have done below.
%%DD the refs for adult health improvements are in my JEEA paper 
%%DD just for later if not now- is it possible to estimate effects on 2g health ie do we have infant mortality or anything in the census for the births of the sulfa-exposed women? i dont think so.
%%SB: I am up to here...XXXX
...
%%DD we should really show parallel trends for our 2g outcomes since these tests are outcome specific. 
 
% Here we follow the literature (see for example \citet{BhalotraVenkataramani2014}) in estimating the effect of this health shock in a difference-in-differences model.We are interested in the effect of a mother being exposed to sulfa drugs early in her life (when these were shown to have large effects on morbidity and mortality)on the eventual quality of her children.  
...
%% DD in the displayed eq we show race-specific cohort, state and time effects c s t -- then you also have notation for state and time effects but not cohort. I think with a FE with subscript (rt) i.e. race-specific time effects, you don't also need to display a FE with subscript (t); i hope this is right. if it is, drop the terms that are not race specific. It it is not right then add a fixed effect for cohort.
%%DD are you using the specification in col. 1 of our main table? We show robustness to controls for control-diseases*post37, state income,infrastructure, state trends, region*year. did you include any of this? if not i thinnk it is ok for now if we are pushed for time, but checking, and flagging for revisions.
%%DD in any case i have left XX above for you to indicate what is in teh vector X that is in the displayed eq.

%From equation \ref{TWINeqn:BV} we isolate the effect of a positive maternal health shock (the reduction in rates of pneumonia in the post sulfa era) on child quality as $\phi^q_1$. 

%As described above, in order to estimate $\gamma$---the violation of the exclusion restriction---we need to estimate the twin-mediated effect of this maternal health shock on child quality.  We thus need to estimate a scale parameter capturing the difference in baseline levels of pneumonia between twin and non-twin families. 

%\footnote{As a simple example, if the effect of
...
%%DD i think instead we want to estimate (twin birth) = a + b (post-37*base_pneumonia) + all FE + X. In eq 8 and in the above eq the dep var can be individual level even though RHS treatment is state-year level and s.e. will be clustered by state. This is what we did in our sulfa1 paper. i have for now retained below what you wrote but using baseline means we are not utilizing the quasi exper change created by sulfa.
...
%%DD need to fill in XX to indicate age of exposure; in our paper we had age specific effects from -1 to 16 in bunches.
...
% This flexible difference-in-differences specification relies on the fact that the Biafra civil occurred in Eastern region of Nigeria, and the groups affected by war were the Igbo and other ethnic minorities in this region.  Exposure is thus defined by ethnicity and mother's birth cohort (see \citet{Akreshetal2012} for further details). 
...
% As discussed in the case of US exposure to sulfonamide drugs, this coefficient represents the direct effect of this particular maternal health shock on child quality.  The underlying parameter of interest for inference ($\gamma$) is the twin-mediated effect of a maternal health shock on child outcomes.  In order to estimate this parameter we thus scale using the difference in rates ofexposure to the maternal health shock among twin and non-twin mothers.
...
\end{equation}
%, however this time in an African country. This parameter provides an estimate of the degree of violation of the exclusion restriction and can be used to estimate \citet{Conleyetal2012} bounds.


%*******************************************************************************
\section{Data and Estimation Samples}              \label{TWINscn:data}
\subsection{Data}                                  \label{TWINsscn:data}
%%DD I need to fill in XX from earlier in teh paper as #DHS countries
...
%This education is not the final education of the child, but rather a marker of their current attainment at the time of survey.  
...
%%DD pl check previous sentence- 0 or 1 at bottom of scale?
...
%This is expected given imperfect fertility control and---even were fertility perfectly controlled by families---given that some twins will occur on a family's final desired birth.  
...
%DD please see XX in next sentence for syntax of n_th and when compiled we need to check that the fig are in fact the ones showing twins by b order in an appendix
...
%Such a sample decision is important when estimating the Q--Q trade--off using twins as an instrument. Given that family size is endogenously chosen by parents and rates of twin birth are not constant by birth-order, twin-births will occur more frequently in families that have a higher fertility preference . This point is addressed by (among others) \citet{RosenzweigWolpin1980} and \citet{Blacketal2005} who first suggested as a way to ensure that twin and non-twin families in the sample would have similar 

%-------------------------------------------------------------------------------

\section{Results: Estimates of the Q--Q trade-off}
%DD i added this intro para here and a ref to Currie Moretti and L-Muney and Gield. When i read a full draft i will seek to avoid repetition and i might move this to the Intro. i'll decide later.
...
%%DD Damian i think we need to show results with and without birth order - maybe in an appendix - to see how much birth order changes the coef- and ref Black and Mogstad. Once we have this we should reference that table in para below and state what if anything the order controls do to coef of interest.
%%DD Tables 7 and 8 do not mention the dep var but in 1st row they contain "`fertility"' ie the RHS var. In T9, the first row contains the dep vars. We should make sure that it is immediately clear what is on the LHS and RHS in Tables 7-9 and the format should be broadly consistent. SO one way to do this is that just under the Table title you have a subtitle. In T7,T8 the subtitle will be "`Dependent variable: Years of education (z-score). In T9, the subtitle will be "`Dependent variable: Years of education (z-score) and Health status (0-5 scale)"'. I prefer years of education z-score to school z-score as school could be a test score or a level of attainment or years.
%%DD The OLS table notes should contain a list of controls in Base, +H and S. Please check that the list i provide below is correct and that they are incl as FE as i suggest
%%DD The IV table does list the base controls but it needs to specify what is added in +H and S exactly too. T1 shows BMI and low-BMI -- which of these is in controls
%%DD Do we not control for race and parity as indicatedin the science paper? if we do this needs to be added to notes to all tables. i have included this below please check. sorry if all this is in earlier parts of the paper i have not checked yet
%%DD In case challenged by space i think you can remove details of 2+ etc and replace with ref to teh Estimation Samples section of hte paper.
...
%%DD T1 does not include race (US) or ethnicity (DHS) which are predictors of twins.? Do we not include wealth in the DHS i thought at one time we did. are wealth/income available in nhis.?
...
%%DD we should ideally formalize the Altonji Oster bounding in relation to the OLS-IV and Conley bounding. i will have a go at this above just flagging now in case i forget later.
...
%%DD i am not sure what is meant by the observable R2 - do you mean the actual R2 in the eq with all relevant observables included (ie S+H col) ? we should rewrite previous sentence.
Our estimates suggest that, using a fairly
demanding maximum R-squared assumption, 
%%DD it is not obvious what demanding means here. If instead of maxR2=2 (actual R2) we had that it is 1.5 (rather than 2) then would the Oster stat be smaller or bigger than 0.55-0.65? let's clarify
...
%%DD again i think it is worth being more explicit. does the last sentence above refer to the Oster ratio being 1? I am guessing that's what equal selection is. But i am not sure whether if we find 0.55 instead of 1 --we have a less informative criterion- maybe paraphrase?
...
%%DD do we return to the Oster Altonji statistics later. we should but i wasnt sure we do.
%%DD if we wanted to use the Oster Altonji  statistics to create bounds i think we need to go one step further. these stats tell us the ratio of unobs to obs that would drive the coef to zero. So we need to translate this into intervals around beta. if we are into the bounds business- the whole of part 2 is focused on this maybe we should take this additional step?
%%SB: ****

Panel B of table \ref{TWINtab:OLS} presents the OLS estimates with and without controls for maternal characteristics, along with the Altonji and Oster statistics for the US sample. As discussed, we now examine not just the Z-score for educational attainment %%for comparison with the DHS results
%%DD is it years or grade for age ? if latter it is not really comparable with the dhs which is years?
but also an indicator of child health.  % and the second a variable recording whether parents consider their children to be in excellent health (measured in the NHIS data).  We focus on both variables given that grade retention in the US is a less relevant measure of child quality than in the developed country sample, however note that as discussed earlier, between 2 and 6\% of children are held back at least one grade in primary school in the US.
For education, the baseline estimates imply that an additional child within the family is
associated with a 2.3 to 4.4\% standard deviation reduction.  %(in the case of the smaller 4+ group this is not statistically significant).  
%%DD you had 1-4.4 which i changed to 2.3-4.4, please check. Also in the 4+ group the baseline coef is signific.
...
A similar pattern of results emerges using child health. %is in excellent health, 
%%DD i can check later as you probably explain in sections i have not carefully read but i think we must explain somewhere if this is really prob(excellent health) or if it is for each child any number in the 0-5 or 1-5 scale, in which case it is as much prob(bad health) and we should probably just call it health. Calling it excellent health implies it is binary: 1 if excellent and otherwise 0. Also how do we interpret these coefs? they are no longer in s.d. so i think we should be explicit on how to interpret a number like 2.8 percent below.
The unadjusted OLS estimates imply that an additional sibling reduces the health of older siblings by between 1.1 and 2.8\%. %%DD you had 0.3 and 2.8\% but the numbers in T7 are diff so i changed, pl check 
%%DD i have added discussion of the bias adjustment here and elsewhere as i think this is quite key to our analysis.
...
%The interpretation of the \citeauthor{Altonjietal2005} and \citeauthor{Oster2013} statistics in panel A are identical to the case of the developing country estimates laid out above.
...
%%DD i do not understand the prev sentence - would that not be if the statistic was 1? Is the idea that 0.7 and 0.8 are close to 1? I changed the language to use "`could drive"' but pl check
%We now turn to estimating a more informative set of bounds using the (flawed) twin instrument.
%%DD i am not sure we can say in a formal way that Conley bounds are more informative?
...
%%DD I have to read Mogstad Wisall again to see if their point about non linearity informs or links to this, i think not. If not maybe we can think about an interpretation of this? but i will first move on to see whether these patterns hold for iV which is where we pay more attention to the #s
%%DD minor point but i know i am sometimes using a hyphen inside trade off and sometimes not and i should check at the end for consistency after checking what is conventional (though in life it is good not to be conventional :-))
...
IV estimates using the twin instrument are in %panel B 
...
%%DD i might go back to Angrist to see what it was about their dsicussion of their 1st stage that seemed interesting,this is just a flag for me.
In panel B, the first column (``Base'') column for each parity group presents estimates of $\hat\beta_1$ from equation
\ref{TWINeqn:IV1} using the current state of the art twin-IV 2SLS estimator. In each of the three samples, in line with the findings of recent studies (\citet{Angristetal2010,Blacketal2005,Caceres2006, FitzsimonsMalde2014}), %%DD i hope i havent messed up the references syntax here
we find no significant Q--Q trade-off. %In table \ref{TWINtab:DHSall} the baseline estimates with no controls for maternal health or socioeconomic variables range from -0.5\% to -3\% of a standard deviation, all statistically insignificant.
...
%Moving from baseline controls to full socioeconomic and health controls (``+S\&H'') in table \ref{TWINtab:DHSall} there are two particularly notable features.  Firstly, in nearly all cases inclusion of additional maternal health predictors partially corrects for twin selection and increases the magnitude of the Q--Q parameter estimate $\hat\beta_1$.  This is precisely in line with hypothesis 2, and the IV selection inequality in section \ref{sscn:validIV}. 
...
%%DD above sentence needs editing, right now i am not sure if or where we detail what H and S are. Since there is a lot of detail here we can consider (later) whether to move this detail to the Methods section -- in a para where we explain that indicators of health are selected to be pre-pregnancy. then here we just reference that section.
%Secondly, it is noteworthy that even with only partial controls for health and socioeconomics included in the final column, in 2 out of the 3 parity groups (3+ and 4+) we that observe a statistically significant trade-off emerges, at each case at the 5\% level.  In the following section we return to this estimate and how to fully estimate bounds despite not fully observing all maternal health variables.
...
%Results for both variables (education Z-score and reported as being in excellent health) show similar qualtitative patterns to those reported based on DHS data.  In nearly all cases examined, the inclusion of health and socioeconomic controls results in IV \emph{point} estimates which move further away from zero. In nearly all cases examined, at all parity levels in both DHS and NHIS, the movements of point estimates is precisely as hypothesised in the series of inequalities described in hypotheses 1 and 2, suggesting considerable improvements in IV estimates from accounting for twin endogeneity.  In the case of self reported health status, the inclusion of additional twin predictors is sufficient to result in statistically significant evidence in favour of the Q--Q trade-off for twins at birth-order three, while for education Z-score, a significant Q--Q trade-off emerges for the 2+ group despite the imprecise standard errors ofestimated coefficients. While for the other NHIS subgroups, the effect of a twin does not always results in statistically significant results, it is interesting to note that the point estimates become more negative, and in some cases move a positive baseline point estimate to a negative final point estimate when partially controlling for maternal health.

%More generally, it is not surprising that confidence intervals on these NHIS point estimates are wide.  It is well recognised that IV estimates are much less precise than their OLS counterparts, and this is more true with the case of the twin instrument, where a small ($\sim$2\%) of the population are twins. The precision of twin estimates is discussed further in \citet{Angristetal2010}. While in the DHS data we are able to produce tighter confidence intervals given the large population of births, in USA NHIS data, despite the fact that point  estimates fully support the hypothesis that a Q--Q trade-off does emerge upon  correcting for non-random twin births, the relatively small sample size makes  these estimates less precise than their DHS counterparts.

%*******************************************************************************
%%DD pl check changed title
\subsubsection{Heterogeneity: Unadjusted and Adjusted IV Estimates}
%%DD i added RZ2010 XX below
...
Allowing for this, we find heterogeneity across gender and birth order. %%DD Damian by order do you mean order at which twins occur and hence the 2+, 3+ sample comparison below? rather than order of the subject or index child for whom quality is measured? we should clarify for the reader too. if this is about order at which twins occur then it is now already discussed above in the main results and i would drop it here rather than bring that discussion down mainly because the differences across 2+ 3 + are not significant and also because there is in fact no clear pattern (I have rewritten what you had written below- because i think there is no clear LDC/US distinction in these findings
...
%Empirically, we find that estimates of the Q--Q trade-off are heterogeneous across birth orders, country income level, and the gender of the child affected by the additional birth. In the developing country sample, the magnitude and significance of the results is lowest when considering the effect on a first-born child moving from two to three siblings (the 2+ group), and significantly largerat higher parities.  In the lower fertility (US) example the effect is concentrated on lower birth orders. The IV results for the US suggest largest effects of fertility shocks at parity two (on education), and at parity thee (on health).
%%DD i need to add and discuss results for gender here. We need to add a ref to the table showing gender het. Shall we revisit fees het? It would have been really nice to show! i can only think of looking at the data and removing an outlier or something like that might spring our results into significance- there was something there when you did this for essen but  not significant.

%%Sonia in middle of editing this section
...
%%DD I will read Dehejia, i read abstract and sent to you but never read the paper. from what you write below it seems we should remove the footnote and add Dehejia to para 1 of this sec where we also cite RZ and Aizer etc.- ifthe point is het in results? 
