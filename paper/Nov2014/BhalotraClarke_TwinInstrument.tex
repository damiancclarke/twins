...
child quality is increasing in child quality and \emph{vice versa}.  By comparing 
those families who unexpectedly produced additional children to those who always 
...
This work which further probes the Q--Q trade-off is important, especially in 
the developing country setting on which we place considerable focus. In order to 
...
Typically, such a treatment requires either focusing on offspring born before
...
%*******************************************************************************
Finally, the twin instrument is not without critique for other reasons. 
...
to bounds estimation in section \ref{TWINsscn:resultBounds}.
...
We are able to piece these requirements together from two principal datasets: 
firstly, the Demographic and Health Surveys, which have been applied over 20 
years in a range of developing countries, and secondly, the United States 
National Health Interview Surveys (NHIS).  In what follows, we describe the
characteristics of these datasets which we will use for our primary analysis.
A comprehensive description of all data, including that which is used only to
illustrate twinning and its relation to maternal characteristics, is provided in
data appendix \ref{TWINscn:dataApp}.
...
samples are defined $\forall\ n \in \{2, 3, 4, 5\}$ such that they include 
...
families are larger than non-twin families. Figure \ref{TWINfig:famsize} 
