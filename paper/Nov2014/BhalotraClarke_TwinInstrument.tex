...
values for $\gamma$, we can form more precise bounds by estimating $\gamma$
more precisely.  As we discuss in appendix \ref{TWINscn:gamma}, this requires
us to estimate the effect of maternal health shocks on twinning, and the effect
of maternal health shocks on child quality.  By taking the ratio of these, we
can calculate directly the partial correlation between twinning (on average
healthy mothers), and child quality.

Add in equations

Add in table

Add in figures with lines (actually just on old figures)

Add in figure (as appendix) of distribution and Kolmogorov-Smirnov.
...
\end{equation}
where $\phi$ is the effect of access to sulfanide drugs on outcome variable 
...
+\varphi^t\mathbf{X}_{st}+\lambda^t_{rc}+\theta_s^t\times\eta^t_t+\varepsilon^t_{stc} \nonumber \\
educ_{stc} &=& \alpha^q + \phi^q (Post_t\times basePneumonia_s) +\theta^q_{rs} +\eta^q_{rt}
+\varphi^q\mathbf{X}_{st}+\lambda^q_{rc}+\theta^q_s\times\eta^q_t+\varepsilon^q_{stc} \nonumber
\end{eqnarray}
where superscript $t$ refers to coefficients in the twin regression while $q$ refers to
quality.  We can thus causally estimate $\phi^t=\frac{\partial twin}{\partial bP}\big|_{X}$ and
...
assumption holds.  Thus, we can (seperately) estimate the effect of positive maternal health shocks 
on twinning, and the effect of positive maternal health shocks on child quality.
...
\end{equation}
which is our estimand of interest.  

In order to estimate (\ref{TWINeqn:realgamma}) we turn to US census data described in
\citet{BhalotraVenkataramani2014}, to estimate $\phi^t$ and $\phi^q$, and calculate the 
ratio of these estimates to form an estimate of $\gamma$, which we denote $\hat\gamma$.  
