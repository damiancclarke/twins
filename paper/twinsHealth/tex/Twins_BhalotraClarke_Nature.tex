...
We collected data on 18,652,028 births, of which 539,544 (2.89\%) are twin births. We accessed data that contain indicators of maternal health and health-related behaviours in addition to age, parity, race and ethnicity, which are known determinants of twin birth\cite{Bulmer1970}. %A full discussion of this data is available in the online methods section, and its coverage is presented in extended data figure 1. %Damian i would keep the preceding sentence but i have left it commented out since i think you had a reason to keep it out.
%%%%% Sonia We can certainly keep.  The only reason I had edited out earlier was for space.
...
The displayed coefficients express the effect of a 1 standard deviation ($\sigma$) increase in an indicator of maternal health on the change in the likelihood, in percentage points (pp), of a mother giving (live) birth to twins (unstandardized estimates are in extended data table E1). For every reported indicator, p$<$0.01 unless otherwise stated. %The percentage of twins in the population with which these marginal effects should be compared ranges from 2.10 to 2.84. i think this is clear in tables and global share was indicated before %Damian: For every rep. indicator p<0.001 unless stated % (unstd are in TES record?)
...
Our estimates for the USA use the universe of mothers giving birth between 2009-2013, excluding those who reported using Artificial Reproductive Technology (ART) on account of the documented positive association of ART with the likelihood of twin births\cite{Vitthalaetal2009}. A 1$\sigma$ increase in rates of smoking in each trimester is associated with a 0.20-0.25 pp reduction in the risk of twin birth (p$<$0.001 in each case). Smoking in the third trimester imposes the largest reduction, consistent with evidence that the adverse effect of smoking on birth weight (a marker of foetal health) is largest in the third trimester\cite{Bernsteinetal2005}; also see table E3 %Damian this is the new table regressing birthweight on smoking.
...
%Damian this is the new table on ART women
...
%Our hypothesis is similar in spirit as both hypotheses are about maternal condition and foetal miscarriage, but TW focused on sex of birth while we focus on twins at birth. Damian- see if you want to keep this commented out sentence.
...
%Damian: since we adjust in fig 2 and fig 1 for fertility and age, do it here too? Standardization and controls as in Table 1?  Notes to Fig 3 mention Unadjusted rates of miscarriage which suggests no controls?
%%%%% Sonia: You are right that we do it though this is in the extended data figure 4 (the conditional results).  The findings are largely similar, so we could also replace it here if we want.
%Damian i have added foetal deaths occuring at or after 20 weeks..
\label{fig:mech}
\end{figure}
%\end{spacing}

%%%%% Sonia: We just need to make a definitive choice here on standardised vs unstandardised (I add a bit in the email...)
\begin{figure}[htpb!]
\begin{subfigure}{.5\textwidth}
