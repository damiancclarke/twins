...
%%%%It is twin babies, so you are correct in saying it is the latter.
  
%-- use\cite{Hoekstraetal2008} - i took this out as it shows height,BMI (and age, parity) are corr with twinning and smoking etc are not. NEED to drop from ref list.
  ...
  %%%%%Even though we talked, I write here just for the record:
  %%%%%Abstract: they state ``approximately 200 but certainly no more than 300''
  %%%%%Main text: 1,500
  %%%%%Methods: Up to 3,000

Twin births are often construed as a natural experiment in the social and natural sciences\cite{Boomsmaetal2002,Poldermanetal2015}. In behavioural genetics, demography and psychology, monozygotic (MZ) twins are studied to assess the importance of nurture relative to nature\cite{Thorndike1905,Boomsmaetal2002,Poldermanetal2015,Phillips1993,BouchardPropping1993,McClearnetal1997,Nisen2013}. In the social sciences, twin births are also used to denote an unexpected increase in family size which assists causal identification of the impact of fertility on investments in children and on women's labour market participation\cite{WolpinRosenzweig2000,RosenzweigWolpin1980,AngristEvans1988}. A premise of these studies is that twin births are quasi-random. %or independent of characteristics of the mother that influence the environment in which children are reared, the investments children receive or the mother's preferences over labour supply. %i commented this out as it looked too long and complicated for an abstract we explain this in simpler words in our concluding para. 
...
We present new population-level evidence that challenges this premise. Using individual data for more than 18 million births (of which more than half a million are twins) in 72 countries, we demonstrate that mother's health is systematically positively associated with the probability of a twin birth. This holds for a range of indicators of mother's health including not only height and weight but also anemia, hypertension, diabetes, asthma, smoking and the consumption of alcohol and drugs during pregnancy, the mother's diet and indicators of prenatal care availability. We are agnostic on the question of whether twin conceptions are random, our hypothesis being that carrying twins to term is particularly demanding, so stressors of maternal health lead to selective miscarriage of twins. We establish this using US vital statistics data on foetal death. We also show that the association of maternal health and twin births is evident in richer and poorer countries, that it is evident even when we use a sample of women who do not use IVF, and it holds conditional upon mother's age and parity. Our findings suggest that estimates using twins to identify impacts of additional children on family behaviours will tend to be biased. They also suggest a new reason why twin studies will tend to have limited external validity.
...
%for example i added "`in population level data"' because we need to distinguish ourselves from papers like Hoekstraetal which we need to drop from bib as i took it out above.
\end{abstract}
...
We collect data on 18,652,028 births, of which 539,544 (2.89\%) are twin births. We accessed data sources that %contain indicators of maternal health and health-related behaviours in addition to age, parity, race and ethnicity, which are known determinants of twin birth\cite{Bulmer1970}. %A full discussion of this data is available in the online methods section, and its coverage is presented in extended data figure 1. %Damian i would keep the preceding sentence but i have left it commented out since i think you had a reason to keep it out.
%%%%%We can certainly keep.  The only reason I had edited out earlier was for space.
...
The displayed coefficients express the effect of a 1 standard deviation ($\sigma$) increase in an indicator of maternal health on the change in the likelihood, in percentage points (pp), of a mother giving (live) birth to twins. %The percentage of twins in the population with which these marginal effects should be compared ranges from 2.10 to 2.84. i think this is clear in tables and global share was indicated before 
...
%Damian extended data table 5 is four words and we refer to many such tables so can we compress this language or does Nature require it. for eg can we say Table E5 where it is clear that E refers to teh extended data tables?
%%%%%I have now labelled them as E#.  You're right that this was costing us many words!
...
%[IS THERE A REFERENCE ON THE EFFECT OF SMOKING BY TRIMESTER?  I HAVEN'T FOUND ONE YET.] Damian when i wrote my comments on the Nature article in a word file it was based on reading a pdf print so i had not seen this remark in the tex file, sorry about that
%%%%% Of course no problem. I should have put this in a more useful place!
...
%%%%%Thanks on the years.  I made a mistake in the table on the numbers for Avon!  I'd accidentally pasted in the same numbers as DHS, so you're right in being surprised, as Avon is the smallest sample of all, at approximately 10,000 children.
...
%\captionsetup{labelformat=empty}
\vspace{5mm}
\caption{\textbf{The positive association of mother's height and twin birth is evident at all income levels}: {\footnotesize The correlation of the average height differential between twin and singleton  mothers in a country with the country's log GDP per capita is plotted. %The log of GDP per capita comes from the World Bank Data Bank (indicator NY.GDP.PCAP.PP.KD), and is expressed at Purchasing Power Parity. I took this out, it can go in Methods/data.  
